  ...
  let value = ref('');
  let currentTransactionSelected = ref(null);
...
const props = defineProps({
  ...
  const openModalDetailRequest = (unique_id) => {
      ...
      currentTransactionSelected.value = unique_id;
  }

  const isModalOpenRef = toRef(props, 'isCurrentDetailRequestModalOpen');


  watch(isModalOpenRef, (newValue) => {
    if (!newValue) {
      currentTransactionSelected.value = null; 
    }
    console.log(currentTransactionSelected.value);
  }); 
    ...
    <div class="card mt-8">
      <DataTable :value="transaksidata" paginator :rows="25" tableStyle="min-width: 50rem" class="shadow-md rounded-lg overflow-hidden">
        ...
        <Column header="Rincian">
          <template #body="{ data }">
              <Button @click="openModalDetailRequest(data.unique_id)" rounded icon="pi pi-arrow-right" iconPos="top" />
          </template>
      </Column>
      </DataTable>
    </div>
